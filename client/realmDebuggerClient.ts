...
function serializeRealmObject(obj: any, schema: RealmSchema): any {
  ...
  for (const key of propKeys) {
    ...
    if (val === null || val === undefined) {
      ...
      } else if (val.toString && val.constructor && val.constructor.name === 'ObjectId') {
...
export function initRealmDebugger(
  ...
  console.log(`[RealmDebugger] Initializing inspector for file: ${realmFileName}`);
  ...
  function captureCurrentState() {
    ...
    schemas.forEach((schema) => {
      try {
        ...
        console.warn(`[RealmDebugger] Failed to read schema data for ${schema.name}:`, err);
        data[schema.name] = [];
      }
    });
  ...
  function connect() {
    ...
    console.log(`[RealmDebugger] Connecting to inspector server at ${serverUrl}...`);
    ws = new WebSocket(serverUrl);

    ws.onopen = () => {
      console.log(`[RealmDebugger] Connected to inspector server successfully.`);
      isConnected = true;
    ...
    ws.onmessage = async (event) => {
      try {
        ...
        if (message.event === 'FORCE_REFRESH') {
          console.log('[RealmDebugger] Force refresh request received from dashboard.');
          sendUpdate('REFRESH', 'ForceRefresh');
        } else if (message.event === 'ADD_RECORD') {
          const { schemaName, record } = message;
          console.log(`[RealmDebugger] Dynamic request to add record to schema "${schemaName}":`, record);
          
          try {
            realm.write(() => {
              const schemas: RealmSchema[] = realm.schema || [];
              const schema = schemas.find((s: any) => s.name === schemaName);
              if (!schema) throw new Error(`Schema "${schemaName}" not found in database.`);
              ...
              for (const key of Object.keys(properties)) {
                ...
                const baseType = propType.endsWith('?') ? propType.slice(0, -1) : propType;
                
                if (baseType === 'objectId' || (typeof propDef === 'object' && propDef.objectType === 'ObjectId') || key === '_id') {
                  try {
                    finalRecord[key] = new (realm.constructor as any).BSON.ObjectId(val);
                  } catch {
                    finalRecord[key] = new (realm.constructor as any).BSON.ObjectId();
                  }
                } else if (baseType === 'date') {
                  const d = new Date(val);
                  if (isNaN(d.getTime())) {
                    throw new Error(`Field "${key}" must be a valid date string.`);
                  }
                  finalRecord[key] = d;
                } else if (baseType === 'int' || baseType === 'double' || baseType === 'float' || baseType === 'number') {
                ...
                } else if (baseType === 'list' || baseType.endsWith('[]') || (typeof propDef === 'object' && (propDef.type === 'list' || propDef.type === 'array'))) {
                  try {
                    if (typeof val === 'string') {
                      if (val.trim().startsWith('[') && val.trim().endsWith(']')) {
                        finalRecord[key] = JSON.parse(val);
                      } else {
                        finalRecord[key] = val.split(',').map((s: string) => s.trim()).filter((s: string) => s.length > 0);
                      }
              ...
              if (schema.primaryKey && !finalRecord[schema.primaryKey]) {
                ...
                const pkType = typeof pkProp === 'string' ? pkProp : (pkProp.type || 'string');
                if (pkType === 'objectId' || schema.primaryKey === '_id') {
                  finalRecord[schema.primaryKey] = new (realm.constructor as any).BSON.ObjectId();
                }
              }
              
              realm.create(schemaName, finalRecord);
            });
            console.log(`[RealmDebugger] Record added successfully to schema ${schemaName}`);
            ws?.send(JSON.stringify({
          ...
          console.log(`[RealmDebugger] Dynamic request to update record in schema "${schemaName}" with PK "${primaryKeyVal}":`, record);
          ...
          try {
            realm.write(() => {
              ...
              if (!primaryKey) throw new Error(`Schema "${schemaName}" does not have a primary key.`);
              ...
              if (pkType === 'objectId' || primaryKey === '_id') {
                pkParsed = new (realm.constructor as any).BSON.ObjectId(primaryKeyVal);
              } else if (pkType === 'int') {
                pkParsed = Number(primaryKeyVal);
              }

              const obj = realm.objectForPrimaryKey(schemaName, pkParsed);
              if (!obj) throw new Error(`Record with primary key "${primaryKeyVal}" not found in "${schemaName}".`);
              ...
              for (const key of Object.keys(properties)) {
                ...
                if (baseType === 'objectId' || (typeof propDef === 'object' && propDef.objectType === 'ObjectId')) {
                  try {
                    (obj as any)[key] = new (realm.constructor as any).BSON.ObjectId(val);
                  } catch {
                    (obj as any)[key] = null;
                  }
                } else if (baseType === 'date') {
                  ...
                  try {
                    if (typeof val === 'string') {
                      if (val.trim().startsWith('[') && val.trim().endsWith(']')) {
                        ...
                        (obj as any)[key] = val.split(',').map((s: string) => s.trim()).filter((s: string) => s.length > 0);
                      }
                    } else if (Array.isArray(val)) {
                      (obj as any)[key] = val;
                    } else {
            ...
            console.log(`[RealmDebugger] Record updated successfully in schema ${schemaName}`);
            ws?.send(JSON.stringify({
              event: 'OPERATION_SUCCESS',
          ...
          console.log(`[RealmDebugger] Dynamic request to add column "${columnName}" (${columnType}) to schema "${schemaName}"`);
          
          try {
            const targetClass: any = realmConfig.schema?.find(
              (s: any) => (s.schema ? s.schema.name : s.name) === schemaName
            );
            if (!targetClass) throw new Error(`Schema class for "${schemaName}" not found.`);
            ...
            targetSchema.properties[columnName] = optional ? `${columnType}?` : columnType;
            ...
            const newRealm = constructor.open ? await constructor.open(realmConfig) : new constructor(realmConfig);
            ...
            console.log(`[RealmDebugger] Dynamic column "${columnName}" successfully added. Reopened Realm with schemaVersion ${realmConfig.schemaVersion}`);
    ...
    ws.onclose = () => {
      console.log('[RealmDebugger] Connection closed. Retrying in 4 seconds...');
      isConnected = false;
      ws = null;
  ...
  function sendUpdate(changeType: 'INSERT' | 'UPDATE' | 'DELETE' | 'REFRESH', schemaName: string) {
      ...
      try {
        ...
        collection.addListener((col: any, changes: any) => {
          ...
          if (changes.insertions && changes.insertions.length > 0) {
            changeType = 'INSERT';
          } else if (changes.modifications && changes.modifications.length > 0) {
            changeType = 'UPDATE';
          ...
          if (changeType) {
            console.log(`[RealmDebugger] Change detected: ${changeType} on schema: ${schema.name}`);
            sendUpdate(changeType, schema.name);
          }
        });
      } catch (err) {
        console.warn(`[RealmDebugger] Could not attach change listener to schema ${schema.name}:`, err);
      }
    });
  }
