  ...
  http
    .get("http://localhost:3005/", (res) => {
      ...
      if (res.statusCode === 200) {
        console.log(
          "✅ Smoke test passed! Server successfully booted and responded.",
        );
        process.exit(0); // Exit success
      } else {
        console.error(
          `❌ Smoke test failed: Expected status code 200, got ${res.statusCode}`,
        );
        process.exit(1);
      }
    })
    .on("error", (err) => {
      console.error(
        "❌ Smoke test failed: Server connection failed:",
        err.message,
      );
      process.exit(1);
    });
