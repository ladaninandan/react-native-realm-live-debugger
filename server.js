    ...
    switch (data.event) {
      ...
      case 'REALM_CHANGED':
        ...
        if (realmCache[data.realmFile]) {
          ...
          realmCache[data.realmFile].lastUpdated = new Date().toLocaleTimeString();
        } else {
          realmCache[data.realmFile] = {
            realmFile: data.realmFile,
            schemas: data.schemas || [],
      ...
      case 'BROWSER_CONNECT':
        ...
        try {
          ...
          console.error('[Debugger] Failed to send state to browser:', err.message);
        }
        break;
      ...
      case 'REQUEST_REFRESH':
        ...
        if (activeAppConnection) {
          try {
            activeAppConnection.write(encodeFrame(JSON.stringify({ event: 'FORCE_REFRESH' })));
          } catch (err) {
            console.error('[Debugger] Failed to forward sync request:', err.message);
          }
        }
        break;

      case 'ADD_RECORD':
        console.log(`[Debugger] Browser requested to add record in schema: ${data.schemaName}`);
        if (activeAppConnection) {
          try {
            activeAppConnection.write(encodeFrame(JSON.stringify({
              event: 'ADD_RECORD',
              schemaName: data.schemaName,
              record: data.record
            })));
          } catch (err) {
            console.error('[Debugger] Failed to forward add record request:', err.message);
          }
        }
        break;

      case 'UPDATE_RECORD':
        console.log(`[Debugger] Browser requested to update record in schema: ${data.schemaName}`);
        if (activeAppConnection) {
          try {
            activeAppConnection.write(encodeFrame(JSON.stringify({
              event: 'UPDATE_RECORD',
            ...
            console.error('[Debugger] Failed to forward update record request:', err.message);
          }
        }
        break;

      case 'ADD_SCHEMA_COLUMN':
        console.log(`[Debugger] Browser requested to add new column to schema: ${data.schemaName}`);
        if (activeAppConnection) {
          try {
            activeAppConnection.write(encodeFrame(JSON.stringify({
            ...
            console.error('[Debugger] Failed to forward add schema column request:', err.message);
          }
        }
        break;

      case 'OPERATION_SUCCESS':
        console.log('[Debugger] Operation succeeded on app client. Notifying browser.');
        broadcastToBrowsers({
      ...
      case 'OPERATION_ERROR':
        console.log(`[Debugger] Operation failed on app client: ${data.message}`);
        broadcastToBrowsers({
...
server.listen(PORT, () => {
  ...
  console.log(`📺 Open http://localhost:${PORT} in your browser to inspect database.\n`);
});
