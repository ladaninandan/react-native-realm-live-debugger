  ...
  if (refreshBtn) {
    refreshBtn.addEventListener("click", () => {
      ...
      refreshBtn.innerHTML =
        '<i data-feather="loader" class="w-4 h-4 animate-spin"></i> Syncing...';
      if (window.feather) window.feather.replace();

      setTimeout(() => {
        refreshBtn.disabled = false;
        refreshBtn.innerHTML =
          '<i data-feather="refresh-cw" class="w-4 h-4"></i> Sync App';
        if (window.feather) window.feather.replace();
      }, 1200);
    });
  }
...
function updateHeaderStatus(state) {
  ...
  const statusIndicatorMobile = document.getElementById(
