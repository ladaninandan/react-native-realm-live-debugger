  ...
  store.subscribeSelector(
    (state) => {
      const file = state.selectedFile
        ...
        : null;
      const activeSchema =
        file && state.selectedSchema
          ? file.schemas.find((s) => s.name === state.selectedSchema)
          : null;
      return {
...
function renderSchemaView(state) {
  ...
  if (!selectedFile || !file) {
    viewer.innerText =
      "Select a schema from the sidebar to view its raw database JSON.";
    const copySchemaJsonButton = document.getElementById(
      "copySchemaJsonButton",
    );
