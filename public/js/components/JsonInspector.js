...
function copyActiveRecordJson() {
  ...
  navigator.clipboard
    .writeText(JSON.stringify(activeRecord, null, 2))
    .then(() => {
      const btn = document.getElementById("copyJsonButton");
      if (btn) {
        btn.innerHTML =
          '<i data-feather="check" class="w-3 h-3 text-green-400"></i> Copied!';
...
function copySchemaJson() {
  ...
  navigator.clipboard
    ...
    .then(() => {
      ...
      if (btn) {
        btn.innerHTML =
          '<i data-feather="check" class="w-3.5 h-3.5 text-green-400"></i> Copied!';
        if (window.feather) window.feather.replace();

        setTimeout(() => {
          btn.innerHTML =
            '<i data-feather="copy" class="w-3.5 h-3.5"></i> Copy JSON';
          if (window.feather) window.feather.replace();
        }, 1500);
      }
      showToast(
        "Clipboard",
        "Schema data JSON copied to clipboard.",
        "success",
      );
    })
    .catch((err) => {
