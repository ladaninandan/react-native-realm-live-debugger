...
export function initDatabaseTable() {
  ...
  store.subscribeSelector(
    (state) => {
      const fileCache = state.selectedFile ? state.currentCache[state.selectedFile] : null;
      const schemaData = (fileCache && state.selectedSchema) ? fileCache.data[state.selectedSchema] : null;
      const schemaInfo = (fileCache && state.selectedSchema) ? fileCache.schemas.find(s => s.name === state.selectedSchema) : null;
      return {
        ...
        activeRecordId: state.activeRecord ? String(state.activeRecord._id) : null
      };
    },
    () => {
...
function renderRecordsTable(state) {
  ...
  const { selectedFile, selectedSchema, currentCache, tableSearchQuery, activeRecord } = state;
  ...
  if (properties.length === 0) {
    headersRow.innerHTML = '<th class="p-4 text-slate-500 italic">No Schema Properties Defined</th>';
    bodyContainer.innerHTML = '<tr><td class="p-8 text-center text-slate-500">Schema has no properties.</td></tr>';
    return;
  }
  ...
  filtered.forEach((record, index) => {
    ...
    const isActive = activeRecord && String(activeRecord._id) === String(record._id);
    ...
    const rowCells = [`<td class="p-4 text-center font-mono text-[11px] text-slate-500">${index + 1}</td>`];
    ...
    properties.forEach((prop) => {
      ...
      if (val === null || val === undefined) {
        ...
        rowCells.push(`<td class="p-4 text-indigo-400 font-mono text-xs max-w-[200px] truncate">${JSON.stringify(val)}</td>`);
      } else {
        rowCells.push(`<td class="p-4 truncate max-w-[250px] font-sans">${String(val)}</td>`);
      }
    });
