...
export function openAddRecordModal() {
  ...
  for (const key of Object.keys(properties)) {
    ...
    let propType =
      typeof propDef === "string" ? propDef : propDef.type || "string";
    ...
    label.className =
      "text-xs font-semibold text-slate-300 flex items-center justify-between";
    label.innerHTML = `
      <span>${key}</span>
    ...
    if (key === "_id") {
      ...
      inputElement.className =
        "w-full bg-slate-950 border border-slate-800 rounded-lg px-3 py-2 text-xs text-slate-500 placeholder-slate-650 focus:outline-none";
      ...
      inputElement.className =
        "w-full bg-slate-950 border border-slate-800 rounded-lg px-3 py-2 text-xs text-slate-200 focus:outline-none focus:border-indigo-500 transition";
    ...
    if (isPrimaryKey) {
      ...
      if (currentVal !== undefined && currentVal !== null) {
        if (typeof currentVal === "object") {
          inputElement.value = Array.isArray(currentVal)
            ? currentVal.join(", ")
