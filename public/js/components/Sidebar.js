...
function renderRealmFiles(state) {
  ...
  fileNames.forEach((fileName) => {
    ...
    const statusBadge = file.status === 'online' 
  ...
  filteredSchemas.forEach((schema) => {
    ...
    btn.innerHTML = `
      ...
      <span class="px-2 py-0.5 rounded-full text-[10px] font-bold ${
        isActive ? 'bg-indigo-600/20 text-indigo-400' : 'bg-slate-800 text-slate-500'
      }">${count}</span>
    `;
    list.appendChild(btn);
  });
...
function selectRealmFile(fileName, state) {
  ...
  document.getElementById('jsonViewer').innerText = 'Select a row to inspect its full JSON content.';
}
...
function selectSchemaTable(schemaName, state) {
  ...
  // Update breadcrumbs
  const navSelectedPath = document.getElementById('navSelectedPath');
  if (navSelectedPath) navSelectedPath.innerText = `${state.selectedFile} > ${schemaName}`;
