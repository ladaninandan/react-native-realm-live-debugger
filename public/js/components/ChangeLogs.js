...
function renderLogs(state) {
  ...
  if (logs.length === 0) {
    container.innerHTML = '<div class="text-slate-500 italic">Listening for live insert, update, and delete events...</div>';
    return;
  }
  ...
  logs.forEach((log) => {
    ...
    logLine.className = 'flex items-center gap-3 font-mono text-[11px] py-1 hover:bg-slate-900/40 px-2 rounded border border-transparent hover:border-slate-800 transition';
